...
#폰트 정의
game_font = pygame.font.Font(None,40)#none 을 적으면 default font 가 쓰임.

#총 시간
total_time = 10

#시작 시간
start_ticks = pygame.time.get_ticks()#시작 시간 tick 을 받아옴.
...
while running:
    ...
    elapsed_time = (pygame.time.get_ticks() - start_ticks)/1000# 지나간 시간을 뜻한다. 지금 시간에서 위에서 정의한 시작 시간을 빼가지고 지나간 시간을 나태내는 형식이다.
    #   나누이 1000 은 기본 단위가 ms 밀리 세컨드 이기때문에 기본 1 초로 만들기 위해서 1000 을 나누는 것이다.
    
    timer = game_font.render(str(int(total_time - elapsed_time)),True,(255,255,255))#ture 는 anti alis 인데 그냥 true 로 놓으면 된다. 뒤에는 생상으로 RGB 값으로 표현 한거다.
